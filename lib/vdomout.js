...
(function() {
  var DataHook, EventHook, NOT_ATTRIBUTES, VDOMOut, VNode, VText, camelize, prepareProps;
  ...
  prepareProps = function(inp) {
    ...
    for (k in inp) {
      ...
      if (k.length > 5 && k.slice(0, 5) === 'data-') {
        ...
        k.length > 2 && (k.slice(0, 2) === 'on' ? props[k] = new EventHook(v) : (NOT_ATTRIBUTES[k] ? props : attrs)[k] = v);
      }
    }
    if (inp["class"]) {
  ...
  VDOMOut.EventHook = EventHook = (function() {
    ...
    EventHook.prototype.hook = function(node, name) {
      var event;
      event = name.slice(2);
      return node.addEventListener(event, this.handler);
    };

    EventHook.prototype.unhook = function(node, name) {
      var event;
      event = name.slice(2);
